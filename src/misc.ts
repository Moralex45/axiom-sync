...
export const bufferToArrayBuffer = (
  ...
  // Ensure the returned value is a real ArrayBuffer (not SharedArrayBuffer),
  // which is required by newer DOM/WebCrypto typings.
 ...
 * @returns
 */
export const arrayBufferToBuffer = (b: ArrayBufferLike) => {
  return Buffer.from(b);
};

export const arrayBufferToBase64 = (b: ArrayBufferLike) => {
  return arrayBufferToBuffer(b).toString("base64");
};

export const arrayBufferToHex = (b: ArrayBufferLike) => {
...
/**
 * https://github.com/axiom-sync/axiom-sync/issues/567
