...
const fromDriveItemToEntity = (x: DriveItem, remoteBaseDir: string): Entity => {
  ...
  // possible prefix:
  // pure english: /drive/root:/Apps/axiom-sync/${remoteBaseDir}
  // or localized, e.g.: /drive/root:/应用/axiom-sync/${remoteBaseDir}
  const FIRST_COMMON_PREFIX_REGEX = /^\/drive\/root:\/[^\/]+\/axiom-sync\//g;
  ...
  // e.g.: /Livefolders/应用/axiom-sync/${remoteBaseDir}
  const SECOND_COMMON_PREFIX_REGEX = /^\/Livefolders\/[^\/]+\/axiom-sync\//g;

  // another report, why???
  // /drive/root:/something/app/axiom-sync/${remoteBaseDir}
  const THIRD_COMMON_PREFIX_REGEX =
    /^\/drive\/root:\/[^\/]+\/app\/axiom-sync\//g;
