...
const DEFAULT_README_FOR_METADATAONREMOTE =
  "Do NOT edit or delete the file manually. This file is for the plugin axiom-sync to store some necessary meta data on the remote services. Its content is slightly obfuscated.";

const DEFAULT_VERSION_FOR_METADATAONREMOTE = "20220220";

export const DEFAULT_FILE_NAME_FOR_METADATAONREMOTE =
  "_axiom-sync-metadata-on-remote.json";

export const DEFAULT_FILE_NAME_FOR_METADATAONREMOTE2 =
  "_axiom-sync-metadata-on-remote.bin";
...
export const deserializeMetadataOnRemote = (x: string | ArrayBuffer) => {
  ...
  let y2: { readme?: unknown; d?: unknown };
  try {
    y2 = JSON.parse(y1) as { readme?: unknown; d?: unknown };
  } catch (e) {
    throw new Error(
  ...
  let y3: string;
  try {
    const parsed = base64url.parse(reverseString(String(y2["d"])), {
      out: Uint8Array,
      loose: true,
    });
    y3 = Buffer.from(parsed).toString("utf-8");
  } catch (e) {
