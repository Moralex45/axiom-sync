...
3. Create a **bucket**, you can leave the default settings, or you can enable the encryption (which is different from what you can set in Axiom Sync):
...
9. Go to Axiom Sync settings in Obsidian and:

   - Choose `S3 or compatibile` in **Remote Service**:
   - Copy `Endpoint` from Backblaze (see 3. above) to `Endpoint` in Axiom Sync
   - From `endpoint` take `region` (eg. `us-east-005`) and paste it in `endpoint` in Axiom Sync
   - Copy `keyID` (see 7. above) to `Access Key ID` in Axiom Sync
   - Copy `applicationKey` (see 7. above) to `Secret Access Key` in Axiom Sync
   - Copy `bucketname` (see 4. above) to `Bucket Name` in Axiom Sync
