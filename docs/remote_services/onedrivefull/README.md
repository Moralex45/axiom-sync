...
* It's a PRO feature of Axiom Sync plugin.
...
## Steps of Axiom Sync subscription
...
3. Go back to your Axiom Sync plugin inside Obsidian, click "Check again" button in PRO settings. So that the plugin knows some features are enabled. In this case, sync with Onedrive (Full) should be detected.

## Steps of Connecting to your Onedrive

After you enabled the PRO feature in your Axiom Sync plugin, you can connect to your Onedrive account now.

1. In Axiom Sync settings, change your sync service to Onedrive (Full).
2. Click Auth, visit the link, go to Onedrive website to start.
3. Follow the instruction on Onedrive, and allow Axiom Sync to connect.
4. You will be redirected back to Axiom Sync plugin.
...
* Onedrive's API does not allow uploading empty files. You can choose to skip them or raise errors in Axiom Sync plugin's settings.

# What's the difference of Onedrive (App Folder) and Onedrive (Full)?

Due to history reasons, Axiom Sync only supported uploading to App Folder back to year 2021. Because that greatly ensured the security and ensure not messing up with others files.

However, repeatedly some users want to sync to arbitrary root folder rather than `/Apps/axiom-sync/` folder. Thus as of June 2024, the new PRO feature is finally developed.
