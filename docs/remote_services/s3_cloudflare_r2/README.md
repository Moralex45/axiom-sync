...
5. In axiom-sync setting page, input the address / bucket / access key / secret key. **Region being set to `us-east-1` is sufficient.** Enable "Bypass CORS", because usually that's what you want.

   Click "check connectivity". (If you encounter an issue and sure the info are correct, please upgrade axiom-sync to **version >= 0.3.29** and try again.)
...
If you encounter an issue and sure the info are correct, please upgrade axiom-sync to **version >= 0.3.29** and try again.
