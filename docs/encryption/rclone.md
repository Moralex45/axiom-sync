...
It's developed based on another js project by the same author of Axiom Sync: [`@fyears/rclone-crypt`](https://github.com/fyears/rclone-crypt), which is NOT an official library from RClone, and is NOT affiliated with RClone.
...
[webdav1]
type = webdav
url = https://example.com/sharefolder1/subfolder1 # the same as the web address in Axiom Sync settings.
vendor = other
...
[webdav1crypt]
type = crypt
remote = nas1test:vaultname # the same as your "Remote Base Directory" (usually the vault name) in Axiom Sync settings
